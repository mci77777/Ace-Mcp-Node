...
export class EnhancePromptService {
  ...
  private async loadPromptFiles(): Promise<void> {
    ...
    try {
      // 定位项目根目录
      // 当前文件是 src/tools/enhancePrompt.ts，编译后是 dist/tools/enhancePrompt.js
  ...
  private parseOpenAIResponse(response: any): string {
    ...
    try {
      ...
      // 如果是我们抛出的错误，直接重新抛出
      if (error instanceof Error && error.message.startsWith('Invalid OpenAI response') || error instanceof Error && error.message.startsWith('Empty content') || error instanceof Error && error.message.startsWith('OpenAI API error')) {
        throw error;
      }
