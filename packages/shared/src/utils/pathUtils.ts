...
export function isWSLPath(filePath: string): boolean {
  ...
  return trimmed.startsWith('\\\\wsl$\\') || trimmed.startsWith('//wsl$/');
}

/**
 * 将 WSL UNC 路径转换为 WSL 内部路径
 * 
 * @param wslPath - WSL UNC 路径（如 \\wsl$\Ubuntu\home\user）
 * @returns WSL 内部路径（如 /home/user）
 * @throws {Error} 当路径格式无效时抛出错误
 */
export function convertWSLToWindows(wslPath: string): string {
  if (!isWSLPath(wslPath)) {
    throw new Error(`Not a valid WSL UNC path: ${wslPath}`);
  }

  const parts = wslPath.replace(/\\/g, '/').split('/').filter(Boolean);
  
  if (parts.length < 3 || parts[0] !== 'wsl$') {
    throw new Error(`Invalid WSL UNC path format: ${wslPath}`);
  }
...
/**
 * 将 Windows 路径转换为 WSL /mnt/ 格式路径
 * 
 * @param winPath - Windows 路径（如 C:\Users\...）
 * @returns WSL /mnt/ 路径（如 /mnt/c/Users/...）
 * @throws {Error} 当路径不是有效的 Windows 路径时抛出错误
 */
export function convertWindowsToWSL(winPath: string): string {
  ...
  // 检查是否为 Windows 盘符路径（如 C:\ 或 C:/）
  const match = trimmed.match(/^([A-Za-z]):[/\\](.*)/);
  if (!match) {
    throw new Error(`Not a valid Windows drive path: ${winPath}`);
  }
 ...
 * @throws {Error} 当路径无效或不可访问时抛出错误
 */
export function validateProjectPath(filePath: string): void {
  if (!filePath || typeof filePath !== 'string') {
    throw new Error('Path cannot be null or undefined');
  }
